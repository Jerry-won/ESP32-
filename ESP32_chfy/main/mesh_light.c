...
void LED2_Init()
{
	gpio_pad_select_gpio(LED_PIN_2);// 选择要操作的GPIO
	gpio_set_direction(LED_PIN_2, GPIO_MODE_OUTPUT);// 设置GPIO为推挽输出模式
}
...
void LED2_ON()
{
	gpio_set_level(LED_PIN_2, 1);// GPIO输出低
}
...
void LED2_OFF()
{
	gpio_set_level(LED_PIN_2, 0);// GPIO输出低
}
